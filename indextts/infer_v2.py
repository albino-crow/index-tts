...
class IndexTTS2:
    ...
    def _load_and_cut_audio(
        ...
        """
        Load and cut audio from file path or BytesIO object.

        Args:
            audio_path: File path (str) or BytesIO object containing audio data
            max_audio_length_seconds: Maximum audio length in seconds
            verbose: Whether to print verbose output
            sr: Target sample rate (if None, uses original sample rate)

        Returns:
            tuple: (audio tensor, sample rate)
        """
        # Check if audio_path is a BytesIO object or file-like object
        if isinstance(audio_path, BytesIO) or (
            hasattr(audio_path, "read") and hasattr(audio_path, "seek")
        ):
            # For BytesIO objects, use soundfile to read directly
            audio_path.seek(0)  # Reset to beginning

            # Read audio data using soundfile
            data, samplerate = sf.read(audio_path)

            # librosa prefers float32 so convert if needed
            ...
            # Optional: resample for librosa
            if sr is not None and samplerate != sr:
                audio = librosa.resample(data, orig_sr=samplerate, target_sr=sr)
                actual_sr = sr
            else:
                audio = data
                actual_sr = samplerate
        else:
            # Handle file path (string) - use librosa directly
            if not sr:
                audio, actual_sr = librosa.load(audio_path)
            else:
                audio, actual_sr = librosa.load(audio_path, sr=sr)

        # Convert to tensor
        audio = torch.tensor(audio).unsqueeze(0)
        max_audio_samples = int(max_audio_length_seconds * actual_sr)
